...
namespace SkladDB
{
    public partial class In_inv_edit : Form
    {
        ...
        private void btn_ok_Click(object sender, EventArgs e)
        {
            ...
            for (i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                if (grid_items_in_inv.Rows[i].Cells[7].Value.ToString() == "Изм. (id)") // Изм.(id)
                {
                    ...
                    param = new SqlParameter("@quantity", (int)grid_items_in_inv.Rows[i].Cells[2].Value);
                    command.Parameters.Add(param);
                ...
                if (grid_items_in_inv.Rows[i].Cells[7].Value.ToString() == "Изм. (имя)") // Изм.(имя)
                {
                    command = new SqlCommand("UPDATE Items SET name = @name WHERE id_product = " + grid_items_in_inv.Rows[i].Cells[9].Value.ToString(), connection);
                    param = new SqlParameter("@name", (string)grid_items_in_inv.Rows[i].Cells[1].Value);
                    command.Parameters.Add(param);
                    command.ExecuteNonQuery();

                    command = new SqlCommand("UPDATE Items_in_inv SET quantity = @quantity, price_one =  @price_one, mark_number = @mark_number WHERE id_write = " + grid_items_in_inv.Rows[i].Cells[0].Value.ToString(), connection);
                    param = new SqlParameter("@quantity", (int)grid_items_in_inv.Rows[i].Cells[2].Value);
                    command.Parameters.Add(param);
                ...
                if (grid_items_in_inv.Rows[i].Cells[7].Value == DBNull.Value) // Новое значение
                {
                    if ((bool)ds.Tables[0].Rows[i][8])
                    {
                        ...
                        param = new SqlParameter("@name", (string)ds.Tables[0].Rows[i][1]);
                        command.Parameters.Add(param);
                    ...
                    param = new SqlParameter("@quantity", (int)ds.Tables[0].Rows[i][2]);
                    command.Parameters.Add(param);
            ...
            command = new SqlCommand("UPDATE In_invoices SET id_wrk = @id_wrk, id_org = @id_org, num_invoice = @num_invoice, date_in = @date_in, date_purchase = @date_purchase, notes = @notes, fio_exp = @fio_exp, prof_exp = @prof_exp WHERE id_in_invoice = " + id, connection);
        ...
        private void In_inv_act_Load(object sender, EventArgs e)
        {
            ...
            adapter = new SqlDataAdapter("SELECT Items_in_inv.id_write AS '№', Items.name AS 'Имя', " + 
                                         "Items_in_inv.quantity AS 'Колличество', Items_in_inv.price_one AS 'Цена (ед.)', " +
                                         "Items_in_inv.quantity * Items_in_inv.price_one AS 'Общая стоимость', " +
            ...
            ds.Tables[0].Columns.Add("Новый товар ?", typeof(bool));
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                ds.Tables[0].Rows[i][8] = false;
            ds.Tables[0].Columns.Add("Номер товара (стар.)", typeof(int));
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                ds.Tables[0].Rows[i][9] = ds.Tables[0].Rows[i][6];
            ...
            grid_items_in_inv.Columns[9].Visible = false;
            ...
            command = new SqlCommand("SELECT num_invoice, fio_exp, prof_exp, notes, date_in, date_purchase FROM In_invoices WHERE id_in_invoice = " + id, connection);
            reader = command.ExecuteReader();
            if (reader.HasRows)
            {
                reader.Read();
                ...
                date_in.Text = reader.GetDateTime(4).ToString();
                date_purchase.Text = reader.GetDateTime(5).ToString();
            }
            reader.Close();
        }
        ...
        private void btn_add_Click(object sender, EventArgs e)
        {
            ...
            { 
                ...
                ds.Tables[0].Rows.Add(DBNull.Value, DataTransfer.Item_value, DataTransfer.quantity, DataTransfer.price, DataTransfer.price * DataTransfer.quantity, DataTransfer.str_mark_num, DBNull.Value, DBNull.Value, DataTransfer.is_new_item);
                else
                ds.Tables[0].Rows.Add(DBNull.Value, DataTransfer.Item_value, DataTransfer.quantity, DataTransfer.price, DataTransfer.price * DataTransfer.quantity, DataTransfer.str_mark_num, DataTransfer.id, DBNull.Value, DataTransfer.is_new_item);
        ...
        private void btn_edit_Click(object sender, EventArgs e)
        {
            ...
            if (grid_items_in_inv.CurrentRow != null)
            {
                ...
                DataTransfer.Item_value = (string)grid_items_in_inv.CurrentRow.Cells[1].Value;
                DataTransfer.quantity = (int)grid_items_in_inv.CurrentRow.Cells[2].Value;
                DataTransfer.price = (decimal)grid_items_in_inv.CurrentRow.Cells[3].Value;
                DataTransfer.is_new_item = (bool)grid_items_in_inv.CurrentRow.Cells[8].Value;
                if (grid_items_in_inv.CurrentRow.Cells[6].Value != DBNull.Value)
                    DataTransfer.id = (int)grid_items_in_inv.CurrentRow.Cells[6].Value;
                ...
                In_inv_edit_items itms = new In_inv_edit_items();
                itms.ShowDialog();
                ...
                if (DataTransfer.action)
                {
                    ...
                    ds.Tables[0].Rows[DataTransfer.str_num][8] = DataTransfer.is_new_item;
                    if (DataTransfer.id != -1)
                        ds.Tables[0].Rows[DataTransfer.str_num][6] = DataTransfer.id;
                    else if (ds.Tables[0].Rows[DataTransfer.str_num][6] != DBNull.Value)
                        ds.Tables[0].Rows[DataTransfer.str_num][6] = DBNull.Value;
